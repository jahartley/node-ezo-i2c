...
class ezo {
  ...
  /*
   * Main API
 // remove these with new info,find,status using check add  
 // PH info
 	phInfo() {
 ...
 // PH status
 	phStatus() {
 		return this._status(cmds.PH)
 	}

 // ORP info
 	orpStatus() {
 		return this._status(cmds.ORP)
 	}
 */
 //info(userAdd) 
 	info(userAdd) {
 		return this._checkAdd(userAdd).
 		then((resp) => this._info(resp)).
 		then((resp) => {if (typeof resp === 'undefined') throw new Error(`info(${userAdd}) failed`)}).
 		catch(console.log)
 	}
